...
def check_parameters(worker):
    draft = worker.get_draft_model()
    expected = load_file("/experiment/tiny-peagle/draft-original/model.safetensors")
...
if __name__ == "__main__":
    ...
    if speculative is not None:
        report["parameters"] = llm.collective_rpc(check_parameters)
    Path(f"/experiment/evidence/l20-20260919/tiny-peagle-{args.mode}.json").write_text(
